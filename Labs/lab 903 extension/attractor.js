...
//    var distToMid;
    if(this.id > 2){
     ...
     distToMidBall = this.loc.dist(midBall.loc);
   ...
   if(distToMidBall < 250){
     //add attraction
     this.acc = p5.Vector.sub(midBall.loc, this.loc);
     this.acc.normalize();
     this.acc.mult(0.1);
   }
   if(distToMidBall < 150){ // add repulsion
     this.acc = p5.Vector.sub(this.loc, midBall.loc);
     this.acc.normalize();
     this.acc.mult(0.5);
   }
